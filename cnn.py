...
class CNNClassfier(object):
    def __init__(self, config):
        ...
        self.click_times = tf.placeholder(tf.int32, [None, self.max_len], name="input_click")
        ...
        with tf.variable_scope("discriminator"):
            ...
            with tf.device('/cpu:0'), tf.name_scope("embedding"):
                ...
                self.advertiser_id_embedding = tf.Variable(
                    tf.random_uniform([self.advertiser_id_size, self.ad_embedding_size], -1.0, 1.0),
                    name="advertiser_id")
                self.click_times_embedding = tf.Variable(
                    tf.random_uniform([self.click_times_size, self.ad_embedding_size], -1.0, 1.0),
                    name="click_times")
                self.creative_id_embedding = tf.Variable(
                    tf.random_uniform([self.creative_id_size, self.ad_embedding_size], -1.0, 1.0),
                    name="creative_id")
                self.industry_embedding = tf.Variable(
                    tf.random_uniform([self.industry_size, self.ad_embedding_size], -1.0, 1.0),
                    name="industry")
                self.product_category_embedding = tf.Variable(
                    tf.random_uniform([self.product_category_size, self.ad_embedding_size], -1.0, 1.0),
                    name="product_category")
                self.product_id_embedding = tf.Variable(
                    tf.random_uniform([self.product_id_size, self.ad_embedding_size], -1.0, 1.0),
                    name="product_id")
                self.time_embedding = tf.Variable(
                    tf.random_uniform([self.time_size, self.ad_embedding_size], -1.0, 1.0),
                    name="time")
                ...
                # use id 
                self.ad_id_embeddings = tf.expand_dims(self.ad_id,axis=-1)
                self.advertiser_id_embeddings = tf.expand_dims(self.advertiser_id_embedding,axis=-1)
                self.click_times_embeddings = tf.expand_dims(self.click_times,axis=-1)
                
                self.creative_id_embeddings = tf.expand_dims(self.creative_id,axis=-1)
                
                self.industry_embeddings = tf.expand_dims(self.industry,axis=-1)
                
                self.product_category_embeddings = tf.expand_dims(self.product_category,axis=-1)
                
                self.product_id_embeddings = tf.expand_dims(self.product_id,axis=-1)
                
                self.time_embeddings = tf.expand_dims(self.time,axis=-1)
                
                    
                    
                
                self.embedded_chars = tf.concat([self.ad_id_embeddings,self.advertiser_id_embeddings \
                self.click_times_embeddings,self.creative_id_embeddings,self.industry_embeddings\
                self.product_category_embeddings,self.product_id_embeddings,self.time_embeddings],axis=-1)
                #concat所有的特征向量
                """
                self.W = tf.Variable(
